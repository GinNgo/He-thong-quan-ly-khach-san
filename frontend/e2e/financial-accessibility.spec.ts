...
test.describe('Financial form and dialog accessibility', () => {
  ...
  test('platform refund form is keyboard-operable and announces validation errors', async ({ page }) => {
    await seedSession(page, {
      fullName: 'Financial Accessibility Admin',
      permissions: [],
      roles: ['SUPER_ADMIN'],
    ...
    await page.goto('/admin/platform-refunds', { waitUntil: 'domcontentloaded' });
    const screen = page.locator('app-platform-refunds');
    await expect(screen.getByRole('heading', { level: 1, name: 'Platform refund control' })).toBeVisible();

    const form = screen.locator('form');
    await expectNamedControls(form);
    const transaction = screen.getByRole('textbox', { name: 'Platform transaction' });
    const amount = screen.getByRole('spinbutton', { name: 'Amount VND' });
    ...
    await expectKeyboardFocusIndicator(amount);
    await page.keyboard.press('Tab');
    await expect(reason).toBeFocused();
    await page.keyboard.press('Tab');
    await expect(submit).toBeFocused();
    await expectTextContrast(submit, 'Platform refund submit button');

    await page.keyboard.press('Enter');
    const error = screen.getByRole('alert');
    await expect(error).toBeVisible();
    await expect(error).toContainText(/Transaction and positive integer amount are required/);
  });
