...
export class HotelDetailComponent implements OnInit, AfterViewChecked {
  ...
  hotel:Hotel|null=null; roomTypes:RoomType[]=[]; operationalPolicy:PublicOperationalPolicy|null=null; isLoading=true; pageError=''; roomError=''; selectedRoomType:RoomType|null=null; selectedQuantity=0; hotelId=0;
  ...
  loadHotelData(id:number):void{this.isLoading=true;this.pageError='';this.roomError='';this.hotel=null;this.operationalPolicy=null;this.api.getHotelById(id).subscribe({next:h=>{if(!h){this.showPageError('Không tìm thấy chỗ nghỉ này. Cơ sở có thể đã ngừng hiển thị hoặc đường dẫn không còn hiệu lực.');return;}this.hotel=h;this.loadOperationalPolicy(id);this.changeDetector.detectChanges();this.loadRoomTypes(id);},error:error=>{this.showPageError(error?.status===404?'Không tìm thấy chỗ nghỉ này. Cơ sở có thể đã ngừng hiển thị hoặc đường dẫn không còn hiệu lực.':'Không thể tải thông tin chỗ nghỉ lúc này. Vui lòng thử lại sau.');}});}
  ...
  private showPageError(message:string):void{this.hotel=null;this.roomTypes=[];this.operationalPolicy=null;this.selectedRoomType=null;this.selectedQuantity=0;this.pageError=message;this.isLoading=false;this.changeDetector.detectChanges();}
