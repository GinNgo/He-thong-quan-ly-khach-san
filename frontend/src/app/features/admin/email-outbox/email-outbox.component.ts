...
export class EmailOutboxComponent implements OnInit {
  ...
  readonly canRetry = this.permissions.hasPermission(FunctionCode.AUDIT_LOG, ActionCode.UPDATE);
  failures: EmailOutboxFailure[] = [];
  attemptsById = new Map<number, EmailDeliveryAttempt[]>();
  loading = true;
  error = '';
  ...
  trackById(_: number, item: EmailOutboxFailure): number {
    return item.id;
  }
}
