  ...
  template: `
    <section class="payment-panel" *ngIf="order" aria-labelledby="platform-payment-title">
      ...
      <div class="attempt-card" *ngIf="attempt">
        ...
        <div class="actions">
          <button *ngIf="attempt.environment === 'SIMULATOR' && (attempt.status === 'CREATED' || attempt.status === 'PENDING')" type="button" class="primary" [disabled]="busy" (click)="confirmSimulator()">
...
export class PlatformPaymentPanelComponent {
  ...
  confirmSimulator(): void {
    if (!this.order || !this.attempt || this.busy || this.attempt.environment !== 'SIMULATOR') return;
    this.busy = true;
    this.error = '';
