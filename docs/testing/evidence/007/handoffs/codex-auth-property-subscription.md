...
| T240 | COMPLETE_VERIFIED | `4b4f034` | Backend production compile and 41/41 focused tests including Spring/JPA concurrency 3/3; SQL Server V74 clean/repeat/duplicate fail-closed/checksum verification; frontend 21/21; independent review; `git diff --check` | Mark T240 complete in Feature 007 `tasks.md`; merge PROP-SUB-014 status/evidence into the two shared aggregate inventories; preserve V74 and stop rollout if its duplicate preflight throws. |
| T241 | STOP_GATE_POLICY | Pending policy approval | Spec/task/inventory search confirms no approved primary/co-owner, invitation, last-owner, transfer/removal/relinquishment or subscription billing-responsibility policy | Keep T241 unchecked; merge PROP-SUB-015 as blocked into shared aggregates; obtain explicit product/finance/legal decisions listed in `T241-ownership-policy-stop-gate.md` before implementation. |
