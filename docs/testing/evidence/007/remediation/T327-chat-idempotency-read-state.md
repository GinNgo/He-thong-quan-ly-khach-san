...
| SQL Server migration | Apply `V61__chat_message_idempotency_and_read_state.sql` twice to isolated test database `T326Queue` in `luxestay-cross-cutting-t326-sql` | PASS twice: 4/4 columns, unique filtered identity index, delivery index, state check/default and zero null states |
