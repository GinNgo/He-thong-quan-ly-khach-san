...
$migrationPath = Join-Path (Resolve-Path (Join-Path $PSScriptRoot '..')).Path 'src/main/resources/db/migration/V84__room_type_gallery_integrity.sql'
$runId = '{0}-{1}' -f $PID, (Get-Date -Format 'yyyyMMddHHmmss')
$containerName = 'luxestay-room-type-gallery-{0}' -f $runId
$databaseName = 'RoomTypeGallery_{0}' -f ($runId -replace '-', '_')
...
function Invoke-SqlQuery([string]$Database, [string]$Query) { & sqlcmd -S $server -U sa -P $saPassword -C -b -d $Database -Q "SET QUOTED_IDENTIFIER ON; $Query"; if ($LASTEXITCODE -ne 0) { throw "sqlcmd failed for $Database" } }
...
CREATE TABLE dbo.room_type_images(id BIGINT IDENTITY(1,1) NOT NULL PRIMARY KEY, room_type_id BIGINT NOT NULL, sort_order INT NOT NULL, is_primary BIT NOT NULL);
INSERT dbo.room_type_images(room_type_id,sort_order,is_primary) VALUES(10,5,1),(10,5,1),(10,9,0);
'@
  Invoke-SqlFile $databaseName $migrationPath; Invoke-SqlFile $databaseName $migrationPath
  $normalized = & sqlcmd -S $server -U sa -P $saPassword -C -h -1 -W -d $databaseName -Q "SET NOCOUNT ON; SELECT CONCAT(COUNT(*),':',COUNT(DISTINCT sort_order),':',SUM(CASE WHEN is_primary=1 THEN 1 ELSE 0 END)) FROM dbo.room_type_images WHERE room_type_id=10;"
  if (-not ($normalized | Select-String '^3:3:1$')) { throw 'Gallery normalization failed.' }
  $orderRejected=$false; try { Invoke-SqlQuery $databaseName "INSERT dbo.room_type_images(room_type_id,sort_order,is_primary) VALUES(10,0,0);" } catch { $orderRejected=$true }
  if (-not $orderRejected) { throw 'Duplicate order was accepted.' }
  $primaryRejected=$false; try { Invoke-SqlQuery $databaseName "INSERT dbo.room_type_images(room_type_id,sort_order,is_primary) VALUES(10,20,1);" } catch { $primaryRejected=$true }
  if (-not $primaryRejected) { throw 'Second primary was accepted.' }
  Write-Output 'Room-type gallery SQL Server validation passed: normalization, idempotence, unique order and single primary.'
} finally {
